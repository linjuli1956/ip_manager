...
#### 技术改进
- **风扇转速徽章系统**：新增_fan_badge方法，为风扇转速提供颜色渐变逻辑
...
- 集成 OpenHardwareMonitor（DLL/WMI），显示温度与风扇转速；支持"实时刷新"与刷新间隔选择
- 顶部新增"复制信息"按钮，一键复制全部硬件信息文本
- 显示器详情：名称、厂商、序列号、物理尺寸（cm）、对角线（英寸）、分辨率与刷新率
...
- 防火墙规则改用自有命名（IPM_ICMPv4_*），避免本地化命名与策略差异导致的兼容性问题：
...
#### 新增/变更
- 更换应用图标为可爱的二次元风格（粉紫渐变+大眼腮红），窗口左上角图标同步更新
- 右侧布局进一步收窄，左侧保持不变
- 新增两种网络测试工具：
  - **内网测试（ping）**：默认取当前选择网卡的网关地址，一键在新cmd窗口执行 `ping <网关> -t`
  - **网络测试（ping）**：默认 `www.baidu.com`，执行 `ping 目标 -t`
- 将"IP操作"按钮组集成进"IP配置"和"额外IP地址"页底部（不再单独作为选项卡）
